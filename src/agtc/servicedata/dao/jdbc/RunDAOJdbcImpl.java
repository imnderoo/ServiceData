...
public class RunDAOJdbcImpl extends JdbcDaoSupport implements RunDAO{
	...
	public List getAllRuns() {
		return getJdbcTemplate().query(
				"select RUN_ID,PROJECT_ID,RUN_DATE,NOTE from RUN",
				new QueryRunHandler());
	}
	...
	public Run getRun(Integer RunId) {
		return (Run)getJdbcTemplate().query(
				"select RUN_ID,PROJECT_ID,RUN_DATE,NOTE from RUN where RUN_ID="
				+RunId.toString(),
	...
	public List getRunsByProjectId(int projectid){
		return getJdbcTemplate().query(
				"select RUN_ID,PROJECT_ID,RUN_DATE,NOTE from RUN where PROJECT_ID="
				+projectid,
				new QueryRunHandler());		
	}
	...
	public Integer saveRun(Run run) {
		...
		Integer result = null;
		if(RunId == -1){
			sql = "insert into RUN(project_id,RUN_DATE,note)"
				+" values (?,STR_TO_DATE('"+run.getDate()+"','%Y-%m-%d %H:%i:%s'),?)";
			...
			sql = " update RUN set project_id = ?,RUN_DATE=STR_TO_DATE('"
				+run.getDate()+"','%Y-%m-%d %H:%i:%s'), note = ? where run_id = ?";
