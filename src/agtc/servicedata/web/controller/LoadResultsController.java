...
public class LoadResultsController extends ServiceDataBasicController {
	...
	protected ModelAndView onSubmit(javax.servlet.http.HttpServletRequest request,
		...
		String MutiAssay = RequestUtils.getStringParameter(request, "MutiAssay", "");
		...
		if (MutiAssay != null && MutiAssay.equals("yes")) {
			...
			while ((aLine = br.readLine()) != null) {
				...
				for (int i = 1; i < st.length; i++) {
					...
					if (result == null || result.length() == 0 || result.equals("0")) {
						result = "00";
					}
					Result r = new Result();
			...
			while ((aLine = br.readLine()) != null) {
				...
				/*
				 * String sampleId = st[sampleIdColumnNo-1].trim(); sampleId =
				 * sampleId.replaceAll(" ",""); String assay =
				 * st[assayNameColumnNo-1].trim(); assay = assay.toUpperCase();
				 * String result = st[resultColumnNo-1].trim(); result =
				 * result.replaceAll("\\.",""); result =
				 * result.replaceAll("/",""); result =
				 * result.replaceAll(" ",""); result = result.toUpperCase();
				 */
		...
		try {
			...
			message = " Have successfully loaded the results!";
