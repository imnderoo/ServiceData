...
public class ResultDAOJdbcImpl extends JdbcDaoSupport implements ResultDAO {
	public List getResults(int projectId) {
		return getJdbcTemplate().query(
				"select result_id,sample_id,assay,RESULT.run_id,result as genoType "
						+ " from RESULT,RUN where RESULT.run_id=RUN.run_id and RUN.project_id=" + projectId
						+ " order by sample_id,assay", new QueryResultHandler());
	}

	public Result getResult(int resultId) {
		System.out.println("getResultHere");

		return (Result) getJdbcTemplate().query(
				"select result_id,sample_id,assay,run_id,result as genoType from RESULT where result_id=" + resultId,
				new QueryResultHandler()).get(0);
	}
	...
	public void saveResult(Result result) {
		...
		= new Object[] { res, resultId };
		getJdbcTemplate().update(sql, param);
	}

	public List getResultsByRunDate(int projectId, Timestamp timestamp) {
		String sql = "select result_id,sample_id,RESULT.run_id,assay,result as genoType"
				+ " from RESULT,RUN where RESULT.run_id=RUN.run_id and RUN.project_id=" + projectId
				+ " and RUN.RUN_DATE=? order by sample_id,assay";
	...
	public List getResultsByRunDate(int projectId, String[] timestamp) {
		String sql = "select result_id,sample_id,RESULT.run_id,assay,result as genoType"
				+ " from RESULT,RUN where RESULT.run_id=RUN.run_id and RUN.project_id=" + projectId
				+ " and RUN.RUN_DATE in " + ArrayToString(timestamp) + " order by sample_id,assay";
		// PreparedStatementSetter pss = new GetByRunDatePSSetter(timestamp);
	...
	public List getUniSamplesByRunDate(int projectId, Timestamp timestamp) {
		String sql = "select distinct RESULT.sample_id from RESULT,RUN"
				+ " where RESULT.run_id=RUN.run_id and RUN.project_id=" + projectId
				+ " and RUN.RUN_DATE=? order by sample_id";
	...
	public List getUniSamplesByRunDate(int projectId, String[] timestamp) {
		String sql = "select distinct sample_id from RESULT,RUN "
				+ " where RESULT.run_id=RUN.run_id and RUN.project_id=" + projectId + " and RUN.RUN_DATE in"
	...
	public List getUniAssaysByRunDate(int projectId, Timestamp timestamp) {
		String sql = "select distinct assay from RESULT,RUN " + " where RESULT.run_id=RUN.run_id and RUN.project_id="
	...
	public List getUniAssaysByRunId(int runId) {
		String sql = "select distinct assay from RESULT " + " where run_id=" + runId;
	...
	public List getUniSamples(int projectId) {
		return getJdbcTemplate().queryForList(
				"select distinct sample_id from RESULT,RUN" + " where RESULT.run_id=RUN.run_id and RUN.project_id="
						+ projectId + " order by sample_id", String.class);
	}

	public List getUniAssays(int projectId) {
		return getJdbcTemplate().queryForList(
				"select distinct assay from RESULT,RUN " + " where RESULT.run_id=RUN.run_id and RUN.project_id="
						+ projectId, String.class);
	}

	public List getUniRunDate(int projectId) {
		return getJdbcTemplate().queryForList("select RUN_DATE from RUN where project_id=" + projectId,
	...
	public void loadResults(List results) {
		...
		String sql = "insert into RESULT(sample_id,assay,result,run_id)" + " values (?,?,?,?)";
	...
	public void removeResults(int projectId, Timestamp timestamp) {
		...
		// Deletion of the run should cause a delete cascade to remove all
		// results
	...
	public void removeResults(int runId, String[] assay) {
		String sql = "delete from RESULT where run_id = " + runId + " and assay in (";
	...
	public class MyBatchPreparedStatementSetter implements BatchPreparedStatementSetter {
		...
		// private Log log =
		// LogFactory.getLog(MyBatchPreparedStatementSetter.class);
